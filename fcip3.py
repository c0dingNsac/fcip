...
def FMIP(username, password):
    ...
    for y in z["content"]:
        try:
            ...
            time_stamp2 = datetime.datetime.fromtimestamp(time_stamp).strftime("%x %X")
            if hours > 0:
        ...
        with open('fcip.csv', 'ab') as csvfile:
            csv_entry = '%s;%s;%s;%s;%s;%s;%s%%;%s\n' % (y["deviceDisplayName"], time.strftime("%x %X", time.localtime(time.time())), time_stamp, street_address, y["location"]["latitude"], y["location"]["longitude"], str(float(y["batteryLevel"]) * 100).split('.')[0], y["batteryStatus"])
...
run_loop_ask = raw_input('Would you like to run this search in a loop? (Y/n): ')
if run_loop_ask.lower() != 'y' :
    print 'Seach loop not requested.\n'
